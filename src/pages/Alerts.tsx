      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: "Active", value: stats.active, icon: AlertTriangle, color: "text-accent-yellow" },
            { label: "Critical", value: stats.critical, icon: XCircle, color: "text-accent-coral" },
            { label: "Resolved", value: stats.resolved, icon: CheckCircle, color: "text-accent-emerald" }
            ...
            <div className="space-y-4">
              {filteredAlerts.map((alert, index) => {
                ...
                return (
                  <GlassCard 
                    key={alert.id} 
                    className={`${getSeverityColor(alert.severity)} border animate-fade-in`}
                    style={{ animationDelay: `${index * 0.1}s` }}
                    hover
                  >
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex items-start space-x-3">
                          <div className={`w-10 h-10 rounded-lg bg-gradient-to-r ${
                            alert.severity === "critical" ? "from-accent-coral to-accent-yellow" :
                            alert.severity === "high" ? "from-accent-yellow to-accent-coral" :
                            alert.severity === "medium" ? "from-accent-cyan to-accent-purple" :
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-1">
                              ...
                              <span className={`text-xs px-2 py-1 rounded-full bg-glass-light ${getStatusColor(alert.status)}`}>
                            ...
                            {alert.symptoms && (
                              <div className="flex flex-wrap gap-1 mt-2">
                                {alert.symptoms.map(symptom => (
                                  <span 
                                    key={symptom}
                                    className="text-xs px-2 py-1 rounded-md bg-glass-light text-muted-foreground"
                                  >
                                    {symptom}
                                  </span>
                                ))}
